...
inline unsigned char to_lower(int c)
{
    const static unsigned char table[256] = {
        ...
        56,  57,  58,  59,  60,  61,  62,  63,  64,  97,  98,  99,  100, 101,
        102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115,
        116, 117, 118, 119, 120, 121, 122, 91,  92,  93,  94,  95,  96,  97,
        ...
        182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 224, 225, 226, 227,
        228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241,
        242, 243, 244, 245, 246, 215, 248, 249, 250, 251, 252, 253, 254, 223,
...
struct Request {
    ...
    std::vector<MultipartFormData> get_file_values(
...
struct Response {
    ...
    void set_content_provider(
        size_t length, const std::string &content_type,
...
public:
    ...
    Result Post(const std::string &path, const char *body,
    ...
    Result Patch(const std::string &path, const char *body,
                 size_t content_length, const std::string &content_type);
    Result Patch(const std::string &path, const char *body,
                 size_t content_length, const std::string &content_type,
                 Progress progress);
    ...
    X509_STORE *create_ca_cert_store(const char *ca_cert,
                       ...
                       BindOrConnect bind_or_connect)
{
    ...
    if (hints.ai_family == AF_UNIX) {
        ...
        if (sock != INVALID_SOCKET) {
            ...
            std::copy(unescaped_host.begin(), unescaped_host.end(),
                      addr.sun_path);
...
#ifdef SOCK_CLOEXEC
        auto sock = socket(rp->ai_family, rp->ai_socktype | SOCK_CLOEXEC,
                           rp->ai_protocol);
#else
        ...
        if (ifa->ifa_addr && ifn == ifa->ifa_name &&
            ...
            if (ifa->ifa_addr->sa_family == AF_INET) {
                ...
                if (!IN6_IS_ADDR_LINKLOCAL(&sa->sin6_addr)) {
                    ...
                    if (inet_ntop(AF_INET6, &sa->sin6_addr, buf,
                                  INET6_ADDRSTRLEN)) {
                                           ...
                                           unsigned int h)
{
    return (l == 0) ? h
                    : str2tag_core(
                          s + 1, l - 1,
                          // Unsets the 6 high bits of h, therefore no overflow
                          // happens
    ...
    return prepare_content_receiver(
        ...
        [&](const ContentReceiverWithProgress &out) {
            ...
            if (is_chunked_transfer_encoding(x.headers)) {
                ...
                auto len =
                    get_header_value_u64(x.headers, "Content-Length", 0, 0);
            ...
            if (!ret) {
                status = exceed_payload_max_length
                             ? StatusCode::PayloadTooLarge_413
            ...
            switch (state_) {
                ...
                case 2: {  // Headers
                    ...
                    while (pos < buf_size()) {
                        ...
                        if (start_with_case_ignore(header,
                                                   header_content_type)) {
                            file_.content_type = trim_copy(
                                header.substr(header_content_type.size()));
                        } else {
                                   ...
                                   CToken ctoken, Content content)
{
    for (size_t i = 0; i < req.ranges.size(); i++) {
        ...
        stoken(
            make_content_range_header_field(offset_and_length, content_length));
        ctoken("\r\n");
        ctoken("\r\n");
...
inline bool retrieve_certs_from_keychain(CFObjectPtr<CFArrayRef> &certs)
{
    ...
    CFObjectPtr<CFDictionaryRef> query(
        CFDictionaryCreate(nullptr, reinterpret_cast<const void **>(keys),
                           values, sizeof(keys) / sizeof(keys[0]),
    ...
    const std::string &password, bool is_proxy = false)
{
    ...
    std::string response;
    {
        ...
        if (qop.empty()) {
            ...
            response = H(H(A1) + ":" + auth.at("nonce") + ":" + nc + ":" +
                         cnonce + ":" + qop + ":" + H(A2));
        }
    }
    ...
    if (res.content_length_ > 0) {
        if (req.ranges.empty()) {
            ...
            return detail::write_content(
                strm, res.content_provider_, offset_and_length.first,
    ...
    if (read_content_core(
        ...
        if (!content_type.find("application/x-www-form-urlencoded")) {
            if (req.body.size() >
                ...
                res.status =
                    StatusCode::PayloadTooLarge_413;  // NOTE: should be 414?
                return false;
            }
            ...
            if (detail::is_valid_path(sub_path)) {
                ...
                if (stat.is_file()) {
                    ...
                    res.set_content_provider(
                        ...
                        [mm](size_t offset, size_t length,
                             DataSink &sink) -> bool {
...
inline socket_t ClientImpl::create_client_socket(Error &error) const
{
    if (!proxy_host_.empty() && proxy_port_ != -1) {
        return detail::create_client_socket(
            proxy_host_, std::string(), proxy_port_, address_family_,
            tcp_nodelay_, ipv6_v6only_, socket_options_,
...
inline bool ClientImpl::send_(Request &req, Response &res, Error &error)
{
    ...
    auto se = detail::scope_exit([&]() {
        // Briefly lock mutex in order to mark that a request is no longer
        // ongoing
        std::lock_guard<std::mutex> guard(socket_mutex_);
        socket_requests_in_flight_ -= 1;
                                       ...
                                       Error &error)
{
    ...
    if (res.get_header_value("Connection") == "close" ||
        ...
        // to do these things to the socket if another thread is using the
        // socket.
        std::lock_guard<std::mutex> guard(socket_mutex_);
        shutdown_ssl(socket_, true);
        shutdown_socket(socket_);
                ...
                if (ok) {
                    ...
                    auto ret = compressor.compress(
                        ...
                        [&](const char *compressed_data,
                            ...
                            req.body.append(compressed_data,
                                            compressed_data_len);
                            return true;
                        });
    ...
    if ((res.status != StatusCode::NoContent_204) && req.method != "HEAD" &&
        ...
        if (res.has_header("Content-Length")) {
            if (!req.content_receiver) {
                auto len =
                    std::min<size_t>(res.get_header_value_u64("Content-Length"),
                                     res.body.max_size());
                if (len > 0) {
    ...
    const MultipartFormDataProviderItems &provider_items) const
{
    ...
    return [&, cur_item, cur_start](size_t offset,
        ...
        if (!offset && !items.empty()) {
            sink.os << detail::serialize_multipart_formdata(items, boundary,
                                                            false);
            return true;
        } else if (cur_item < provider_items.size()) {
            if (!cur_start) {
                const auto &begin =
                    detail::serialize_multipart_formdata_item_begin(
            ...
            if (!provider_items[cur_item].provider(offset - cur_start,
                                                   cur_sink)) {
                return false;
            }
                            ...
                            const char *private_key_password)
{
    ...
    if (ctx_) {
        ...
        if (private_key_password != nullptr &&
            ...
            SSL_CTX_set_default_passwd_cb_userdata(
                ctx_, reinterpret_cast<void *>(
                          const_cast<char *>(private_key_password)));
        }
        ...
        if (SSL_CTX_use_certificate_chain_file(ctx_, cert_path) != 1 ||
            ...
            SSL_CTX_set_verify(
                ctx_, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
                nullptr);
        }
    }
}
            ...
            if (detail::parse_www_authenticate(proxy_res, auth, true)) {
                ...
                if (!detail::process_client_socket(
                        ...
                        [&](Stream &strm) {
                            ...
                            return process_request(strm, req3, proxy_res, false,
                                                   error);
                        })) {
...
inline bool SSLClient::load_certs()
{
    ...
    std::call_once(initialize_cert_, [&]() {
        ...
        if (!ca_cert_file_path_.empty()) {
            ...
            auto loaded = false;
#ifdef _WIN32
            loaded = detail::load_system_certs_on_windows(
                SSL_CTX_get_cert_store(ctx_));
#elif defined(CPPHTTPLIB_USE_CERTS_FROM_MACOSX_KEYCHAIN) && defined(__APPLE__)
#if TARGET_OS_OSX
        ...
        [&](SSL *ssl2) {
            ...
            if (server_certificate_verification_) {
                if (server_certificate_verifier_) {
                    ...
                    auto se =
                        detail::scope_exit([&] { X509_free(server_cert); });
