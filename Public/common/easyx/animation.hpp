...
public:
    // 居中锚点、底部居中锚点
    enum class AnchorMode { Centered, BottomCentered };
...
public:
    ...
    void setOnFinished(std::function<void()> onFinished) { this->onFinished = onFinished; }
    ...
    void onRender()
    {
        ...
        rectDst.y = (anchorMode == AnchorMode::Centered)
                        ? static_cast<int>(position.y) - frame.rectSrc.h / 2
                        : static_cast<int>(position.y) - frame.rectSrc.h;
...
private:
    ...
    struct Frame {
        ...
        Frame() = default;
        Frame(IMAGE* image, const Rect& rectSrc) : img(image), rectSrc(rectSrc) {}
        ~Frame() = default;
    };
