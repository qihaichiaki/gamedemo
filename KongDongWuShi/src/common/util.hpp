...
inline void putimageEx(IMAGE* img, const Rect* rectDst,
    ...
    AlphaBlend(GetImageHDC(GetWorkingImage()), rectDst->x, rectDst->y,
               rectDst->w, rectDst->h, GetImageHDC(img),
               rectSrc ? rectSrc->x : 0, rectSrc ? rectSrc->y : 0,
               rectSrc ? rectSrc->w : img->getwidth(),
               rectSrc ? rectSrc->h : img->getheight(), blendFunc);
}
...
inline void playAudio(LPCTSTR id, bool isLoop = false) {
    static TCHAR strCmd[512];
    _stprintf_s(strCmd, _T("play %s %s from 0"), id,
                isLoop ? _T("repeat") : _T(""));
    mciSendString(strCmd, NULL, 0, NULL);
}
...
inline std::string utf8_to_gbk(const std::string& utf8Str) {
    ...
    WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), -1, &gbkStr[0], len, NULL,
                        NULL);
...
public:
    ...
    StrData& operator[](const std::string& key)
    {
        return _map[key];
    }
